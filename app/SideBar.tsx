...
export default  function SideBar() {
  ...
  const pathname = usePathname()
    ...
    return (
      <div>
        {pathname === "/createMeme" || pathname === "/AiMeme" ? 
            "" : 
          <div className={` fixed h-full w-fit ml-5 sm:ml-15 lg:ml-16 xl:ml-32 text-left hidden lg:flex flex-col border-r gap-4 pr-5 pt-32`}>
            <div className=" flex flex-col gap-2 text-md font-bold pb-4">
              <SideBarItems bgColor={ pathname ==="/" ? '#fddde8' : 'white'} textColor={pathname ==="/" ? '#000' : ''} icon="/popular.svg" text="Trending Memes" link="/"/>
              ...
              <SideBarItems bgColor={ pathname ==="/" ? '' : ''} textColor={pathname ==="/" ? '' : ''} icon="/dogieHead.svg" text="AI memes" link="/AiMeme"/>
            </div>
          </div>
          }
      </div>
 ...
 function SideBarItems({icon, text, link, textColor, bgColor}: {icon : string, text:string, link:string, textColor: string, bgColor: string}){
      return(
        <div style={{backgroundColor: bgColor, color: textColor}} className="flex cursor-pointer hover:bg-[#fddde8] text-[#524A4E] hover:text-[#000] p-2 rounded-lg items-center gap-2">
